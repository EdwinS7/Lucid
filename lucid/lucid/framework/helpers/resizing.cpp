...
vec2_t lucid_engine::ui::handle_resizing() {
	...
	bool inside_bounds = g_input.get()->mouse_hovering_rect(m_window_pos.at(m_window_id) + m_window_size.at(m_window_id) - area, area);

	resizing_info& window_info = info.at(m_window_id);
	...
	if (!window_info.resizing && !window_info.outside_bounds && held && inside_bounds) {
		window_info.difference = g_input.get()->m_mouse_pos - (m_window_pos.at(m_window_id) + m_window_size.at(m_window_id));
	...
	vec2_t new_size = (g_input.get()->m_mouse_pos - m_window_pos.at(m_window_id)) - window_info.difference;

	new_size = vec2_t{ std::max(new_size.x, m_window_min_size.at(m_window_id).x), std::max(new_size.y, m_window_min_size.at(m_window_id).y) };
...
bool lucid_engine::ui::is_this_resizing() {
	return info.at(m_window_id).resizing;
}