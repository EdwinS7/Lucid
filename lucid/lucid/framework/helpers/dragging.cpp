...
vec2_t lucid_engine::ui::handle_dragging() {
	...
	bool inside_bounds = g_input.get()->mouse_hovering_rect(m_window_pos.at(m_window_id), m_window_size.at(m_window_id));

	dragging_info& window_info = info.at(m_window_id);
	...
	if (!window_info.dragging && !window_info.outside_bounds && held && inside_bounds) {
		window_info.difference = g_input.get()->m_mouse_pos - m_window_pos.at(m_window_id);
...
bool lucid_engine::ui::is_this_dragging() {
	return info.at(m_window_id).dragging;
}