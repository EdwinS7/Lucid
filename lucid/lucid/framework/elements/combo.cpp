...
void lucid_engine::ui::combo_box(const char* title, int* value, std::vector<const char*> options) {
	...
	if (m_group->element_visible(m_group->m_elements_pos + vec2_t(0, text_size.y), combo_size)) {
		...
		g_renderer->text(g_renderer->m_defualt_font, options.at(*value), m_group->m_elements_pos + vec2_t(3, 1 + text_size.y), m_style->m_text_active);
		...
		if (m_combo_info[m_combo_id].in_use) {
			...
			for (int i = 0; i < options.size(); i++) {
				...
				g_renderer->text(g_renderer->m_defualt_font, options.at(i), bbox + vec2_t(3, 1), *value == i ? m_style->m_accent : m_style->m_text_inactive);
			}
