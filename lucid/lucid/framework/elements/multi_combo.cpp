...
void lucid_engine::ui::multi_combo_box(const char* title, std::vector<bool>* selected, std::vector<const char*> options) {
	...
	if (g_input.get()->rect_clipping_rect(m_elements_pos, combo_size, m_window_pos[m_window_id] + m_group_pos[m_group_id] + vec2_t(0, 26), m_group_size[m_group_id] - vec2_t(0, 26))) {
		g_renderer.get()->filled_rounded_rectangle(m_elements_pos, combo_size, m_style->m_element_inactive, m_style->m_element_rounding, m_multi_combo_info[m_multi_combo_id].in_use ? corner_top : corner_all);
		g_renderer.get()->rounded_rectangle(m_elements_pos, combo_size, m_style->m_element_outline, m_style->m_element_rounding, m_multi_combo_info[m_multi_combo_id].in_use ? corner_top : corner_all);
