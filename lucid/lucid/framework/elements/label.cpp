...
void lucid_engine::ui::label(const char* title, color_t color) {
	...
	// render our element.
	if (g_input.get()->rect_clipping_rect(m_elements_pos, text_size, m_window_pos[m_window_id] + m_group_pos[m_group_id] + vec2_t(0, 26), m_group_size[m_group_id] - vec2_t(0, 26)))
