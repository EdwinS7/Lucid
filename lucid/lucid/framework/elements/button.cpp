...
bool lucid_engine::ui::button(const char* title) {
	...
	vec2_t button_size = vec2_t(m_group_size.at(m_group_id).x - m_style->m_group_padding * 2, 16);
	bool pressed = false;
	...
	if (g_input.get()->rect_clipping_rect(m_elements_pos, button_size, m_window_pos.at(m_window_id) + m_group_pos.at(m_group_id) + vec2_t(0, 26), m_group_size.at(m_group_id) - vec2_t(0, 26))) {
