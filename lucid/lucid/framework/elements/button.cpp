...
bool lucid_engine::ui::button(const char* title) {
	...
	vec2_t button_size = vec2_t(m_group_size[m_group_id].x - m_style->m_group_padding * 4, 16);
	bool pressed = false;
	...
	if (is_element_visible(m_elements_pos, button_size)) {
		// render our element.
		g_renderer.get()->filled_rectangle(m_elements_pos, button_size, m_style->m_element_inactive);
		g_renderer.get()->rectangle(m_elements_pos + vec2_t(1, 1), button_size - vec2_t(2, 2), m_style->m_element_in_outline);
		g_renderer.get()->rectangle(m_elements_pos, button_size, pressed ? m_style->m_accent : m_style->m_element_outline);
