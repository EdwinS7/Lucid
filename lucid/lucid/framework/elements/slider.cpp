...
void lucid_engine::ui::slider_int(const char* title, int min, int max, int* value) {
	...
	if (g_input.get()->rect_clipping_rect(m_elements_pos, slider_size, m_window_pos[m_window_id] + m_group_pos[m_group_id] + vec2_t(0, 26), m_group_size[m_group_id] - vec2_t(0, 26))) {
		g_renderer.get()->filled_rectangle(m_elements_pos, slider_size, m_style->m_element_inactive);
		g_renderer.get()->filled_rectangle(m_elements_pos, vec2_t(g_animations.get()->map(*value, min, max, 6, slider_size.x), slider_size.y), m_style->m_accent);
		g_renderer.get()->rectangle(m_elements_pos + vec2_t(1, 1), slider_size - vec2_t(2, 2), m_style->m_element_in_outline);
		g_renderer.get()->rectangle(m_elements_pos, slider_size, m_style->m_element_outline);
	...
	// apply new position.
	m_elements_pos += vec2_t(0, slider_size.y + m_style->m_group_spacing);
}
