...
void lucid_engine::ui::slider_int(const char* title, int min, int max, int* value) {
	...
	if (g_input.get()->mouse_hovering_rect(m_group->m_elements_pos + vec2_t(0, text_size.y), slider_size) && g_input.get()->is_key_held(VK_LBUTTON) && !another_in_use)
	...
	if (m_slider_info[m_slider_id].in_use) {
		*value = g_math.get()->map(g_input.get()->m_mouse_pos.x - m_group->m_elements_pos.x, 0.f, slider_size.x, min, max);
	...
	if (m_group->element_visible(m_group->m_elements_pos, slider_size)) {
		...
		renderer->filled_rectangle(m_group->m_elements_pos + vec2_t(0, text_size.y), slider_size, m_style->m_element_inactive);
		renderer->filled_rectangle(m_group->m_elements_pos + vec2_t(0, text_size.y), vec2_t(g_math.get()->map(*value, min, max, 0, slider_size.x), slider_size.y), m_style->m_accent);
		renderer->rectangle(m_group->m_elements_pos + vec2_t(0, text_size.y) + vec2_t(1, 1), slider_size - vec2_t(2, 2), m_style->m_element_in_outline);
		renderer->rectangle(m_group->m_elements_pos + vec2_t(0, text_size.y), slider_size, m_style->m_element_outline);
	}
