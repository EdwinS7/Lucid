...
void lucid_engine::ui::popup(const char* title, vec2_t pos, color_t color) {
	...
	// render our element.
	g_renderer.get()->filled_rectangle(pos, vec2_t(4 + text_size.x, 2 + text_size.y), m_style->m_window_background);
	g_renderer.get()->rectangle(pos, vec2_t(4 + text_size.x, 2 + text_size.y), m_style->m_window_outline);
