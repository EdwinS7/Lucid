...
void lucid_engine::ui::handle_tabs() {
	...
	draw_position.x = (m_window_pos[m_window_id].x + (m_window_size[m_window_id].x * 0.5)) - (area.x * m_tabs.size()) * 0.5;
	...
	for (int i = 0; i < m_tabs.size(); i++) {
		...
		if (m_tab == i) {
			...
			g_renderer.get()->text(g_renderer.get()->m_defualt_font, m_tabs[i].m_title, draw_position + vec2_t((-title_size.x + area.x) * 0.5, area.x - 2 - title_size.y ), m_style->m_text_active );
			g_renderer.get()->text(g_renderer.get()->m_primordial_font, m_tabs[i].m_icon, draw_position + vec2_t((-icon_size.x + area.x) * 0.5, area.x - 2 - title_size.y - icon_size.y ), m_style->m_text_active );
		}
		else {
			if (hovered) {
			...
			g_renderer.get()->text(g_renderer.get()->m_defualt_font, m_tabs[i].m_title, draw_position + vec2_t((-title_size.x + area.x ) * 0.5, area.x - 2 - title_size.y ), m_style->m_text_inactive);
			g_renderer.get()->text(g_renderer.get()->m_primordial_font, m_tabs[i].m_icon, draw_position + vec2_t((-icon_size.x + area.x ) * 0.5, area.x - 2 - title_size.y - icon_size.y ), m_style->m_text_inactive);
		}
