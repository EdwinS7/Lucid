...
void lucid_engine::ui::create_window(const char* title, vec2_t pos, vec2_t min_size, vec2_t default_size) {
	...
	g_renderer.get()->text(g_renderer.get()->m_defualt_font, title, m_window_pos[m_window_id] + vec2_t((m_window_size[m_window_id].x * 0.5) - text_size.x * 0.5, 6), m_style->m_text_active);
}
