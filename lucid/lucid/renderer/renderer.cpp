...
void lucid_engine::renderer::render_draw_data() {
	...
	if (!vertex_buffer || vertex_buffer_size < compiled_draw_data.total_vertex_count) {
		if (vertex_buffer) {
	...
	if (!index_buffer || index_buffer_size < compiled_draw_data.total_index_count) {
		if (index_buffer) {
	...
	if (vertex_buffer->Lock(0, (UINT)(compiled_draw_data.total_vertex_count * sizeof(vertex_t)), (void**)&vtx_dst, D3DLOCK_DISCARD) < 0)
		throw std::runtime_error{ "vertex_buffer->Lock error" };
	if (index_buffer->Lock(0, (UINT)(compiled_draw_data.total_index_count * sizeof(std::uint32_t)), (void**)&idx_dst, D3DLOCK_DISCARD) < 0)
	...
	//Figure out how to build geometry buffer here.
	...
	compiled_draw_data.vertices.clear();
	draw_data.clear();
}
