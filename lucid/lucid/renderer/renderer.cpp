...
void lucid_engine::renderer::create_objects( ) {
	if ( !g_graphics.get( )->direct_3d_device )
		throw std::runtime_error { "create_objects error { device is not setup }" };

	D3DXCreateSprite( g_graphics.get( )->direct_3d_device, &font_sprite );
	fonts.push_back( create_font( "Segoe UI", 13, 400, font_flags_t( true, false, false ) ) );
	fonts.push_back( create_font( "Primordial-Icons", 26, 400, font_flags_t( true, false, false ) ) );
}
...
void lucid_engine::renderer::destroy_objects( ) {
	...
	//fonts
	for ( font_t& font : fonts ) {
		if ( font.dx_font ) {
			font.dx_font->Release( );
			font.dx_font = nullptr;
		}
	}
	fonts.clear( );
}
...
void lucid_engine::renderer::render_draw_data( ) {
	...
	if ( !vertex_buffer || compiled_draw_data.total_vertex_count * sizeof( vertex_t ) > vertex_buffer_size ) {
		...
		if ( g_graphics.get( )->direct_3d_device->CreateVertexBuffer( vertex_buffer_size * sizeof( vertex_t ), D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, D3DFVF_XYZ | D3DFVF_DIFFUSE | D3DFVF_TEX1, D3DPOOL_DEFAULT, &vertex_buffer, nullptr ) < 0 )
	...
	if ( !index_buffer || compiled_draw_data.total_index_count * sizeof( std::uint32_t ) > index_buffer_size ) {
		...
		if ( g_graphics.get( )->direct_3d_device->CreateIndexBuffer( index_buffer_size * sizeof( std::uint32_t ), D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, D3DFMT_INDEX32, D3DPOOL_DEFAULT, &index_buffer, nullptr ) < 0 )
			throw std::runtime_error { "CreateIndexBuffer error" };
	}

	IDirect3DStateBlock9* state_block { };
	if ( g_graphics.get( )->direct_3d_device->CreateStateBlock( D3DSBT_ALL, &state_block ) < 0 )
		return;
	...
	g_graphics.get( )->direct_3d_device->GetTransform( D3DTS_WORLD, &last_world );
	g_graphics.get( )->direct_3d_device->GetTransform( D3DTS_VIEW, &last_view );
	g_graphics.get( )->direct_3d_device->GetTransform( D3DTS_PROJECTION, &last_projection );
	...
	g_graphics.get( )->direct_3d_device->SetStreamSource( 0, vertex_buffer, 0, sizeof( vertex_t ) );
	g_graphics.get( )->direct_3d_device->SetIndices( index_buffer );
	...
	for ( const draw_data_t& data : draw_data ) {
		...
		g_graphics.get( )->direct_3d_device->SetRenderState( D3DRS_MULTISAMPLEANTIALIAS, data.anti_alias );
		...
		if ( text_info.setup ) {
			...
			g_graphics.get( )->direct_3d_device->DrawIndexedPrimitive( data.draw_type, start_vertex, 0, data.vertex_count, start_index, data.index_count / 3 );

		start_vertex += data.vertex_count;
		start_index += data.index_count;
	}

	g_graphics.get( )->direct_3d_device->SetTransform( D3DTS_WORLD, &last_world );
	g_graphics.get( )->direct_3d_device->SetTransform( D3DTS_VIEW, &last_view );
	g_graphics.get( )->direct_3d_device->SetTransform( D3DTS_PROJECTION, &last_projection );
...
font_t lucid_engine::renderer::create_font( const std::string font_name, const int size, const int weight, const font_flags_t font_flags ) {
	return font_t( g_graphics.get( )->direct_3d_device, font_name.c_str( ), size, weight, font_flags );
}
